...
from .senior_web_api import Teacher
from . import senior_sample_article

def print_article_evaluation():
    st.write(f"Evaluation_Cost_Time : {st.session_state['Evaluation_Cost_Time']}")

    response = st.session_state['senior_evaluation']
...
def submit_func(requirements, title, word_count, content):
    ...
    st.session_state['senior_history'] = [requirements, title, word_count, content]
...
def get_evaluation(requirements, title, word_count, content):
    ...
    # print(evaluation)
    st.session_state['senior_evaluation'] = json.loads(evaluation)
...
def show():
    ...
    if st.session_state['senior_marking'] == 'working':
        article_insert_container()

    elif st.session_state['senior_marking'] == 'submitted':
        if st.session_state['senior_history'] != []:
            submit_func(*st.session_state['senior_history'])
        else:
